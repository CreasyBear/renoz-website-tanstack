...
export default function ComparisonTable({ products, features, title }: ComparisonTableProps) {
  return (
      ...
      <div className="min-w-[800px] lg:min-w-0">
        <div className="grid grid-cols-[1.5fr_repeat(3,1fr)] gap-4">
          ...
          {products.map((product, i) => (
            <div key={i} className={`text-center p-6 rounded-t-2xl ${
              product.highlight ? 'bg-[var(--black)] text-white relative -top-4 pt-10 shadow-lg' : 'bg-transparent text-[var(--black)]'
            }`}>
              {product.highlight && (
          ...
          {features.map((feature, featIndex) => (
            <div key={featIndex} className="contents group">
              ...
              {products.map((product, prodIndex) => {
                ...
                return (
                  <div key={prodIndex} className={`p-4 flex items-center justify-center border-b ${
                    isHighlight ? 'bg-gray-50 border-gray-200' : 'bg-transparent border-gray-100'
                  } group-last:border-0 ${
                    // Add bottom corners to last row
                    featIndex === features.length - 1 ? (isHighlight ? 'rounded-b-2xl' : '') : ''
                  }`}>
                    {value === true ? (
                      ...
                      <span className={`font-bold ${isHighlight ? 'text-[var(--black)]' : 'text-gray-600'}`}>
                        {value}
                      </span>
                    )}
                  </div>

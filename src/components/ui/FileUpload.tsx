  ...
  const handleFileSelect = async (selectedFiles: FileList | null) => {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      if (!allowedTypes.includes(file.type)) {
        alert(`${file.name} is not an allowed file type. Please upload JPEG, PNG, or PDF files.`)
        continue
      }
      ...
      try {
        ...
        reader.onload = async (e) => {
          ...
          const result = await (uploadWarrantyFile as any)({
            data: {
              warrantyId,
              file: {
                name: file.name,
          ...
          if (result.success && result.url) {
            ...
            onFilesChange((prevFiles: UploadedFile[]) => {
              ...
              const fileIndex = updatedFiles.findIndex((f) => f.name === file.name && f.uploading)
              if (fileIndex !== -1) {
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-xs text-gray-500">
          or click to browse • {Math.round(maxFileSize / 1024 / 1024)}MB per file
        </p>
            ...
            {files.map((file, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    {file.uploading ? 'Uploading...' : file.error ? (

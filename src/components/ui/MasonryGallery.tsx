...
export default function MasonryGallery({ images, title, showRating = false }: MasonryGalleryProps) {
  return (
    <div className="w-full py-12">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
        {title && <h2 className="text-3xl md:text-4xl font-bold text-[var(--black)]">{title}</h2>}
        ...
        {showRating && (
           <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-full shadow-sm border border-gray-100">
             ...
             <span className="font-bold text-sm text-[var(--black)]">4.9/5 from Local Installers</span>
           </div>
        )}
      </div>

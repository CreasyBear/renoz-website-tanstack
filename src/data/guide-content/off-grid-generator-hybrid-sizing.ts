	...
	sections: [
		{
			...
			body: [
				"Off-grid design has one correct sequence: load audit first, then daily kilowatt-hours, then autonomy days, then surge kilowatts, and only then the genset. The full step-by-step maths for the first four steps, with worked examples and the autonomy-day definitions, lives in the [battery sizing guide for off-grid WA](/guides/battery-sizing-off-grid-wa). This page picks up where that maths ends: what the sequence means for the genset, which is the step everyone gets wrong.",
		...
		},
		{
			...
			body: [
				...
				"Stated as posture rather than arithmetic: a stack that covers fewer than two autonomy days on its own is exactly the regime where a properly controlled genset earns its keep as a bridge rather than a crutch. Run your own winter load through the [battery sizing guide for off-grid WA](/guides/battery-sizing-off-grid-wa) to land on the module count, and check the arithmetic against the case study's published stack. Site-specific design, protection and commissioning remain the accredited installer's scope, as always.",

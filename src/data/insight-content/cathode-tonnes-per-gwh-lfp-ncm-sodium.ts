...
const officialCheckTable: InsightTableBlock = {
	...
	note: "Official replies are cell-level consumption (“tonnes of cathode per GWh of cells”). None states whether manufacturing loss is inside the number, and none publishes a single 1,600. Dangsheng’s band covers the yield-adjusted 523 and 622 back-calculations. Dynanonic’s floor is 4.8% above generation-3 LFP; the ceiling is 19% above, which the note reads as a possible specific-capacity margin. Xiangtan’s core business is not LFP — Hunan Yuneng, an associate, is the LFP name — so that line is recorded and then dropped.",
};

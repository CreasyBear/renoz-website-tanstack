...
export const submitInquiry = createServerFn({
  ...
  if (turnstileToken === 'mock-token' || process.env.VITE_DISABLE_TURNSTILE === 'true') {
  ...
  const recipientEmail = process.env.CONTACT_FORM_TO_EMAIL || import.meta.env.CONTACT_FORM_TO_EMAIL || 'sales@renoz.energy'
  const fromEmail = process.env.CONTACT_FORM_FROM_EMAIL || import.meta.env.CONTACT_FORM_FROM_EMAIL || 'RENOZ Energy <noreply@renoz.energy>'

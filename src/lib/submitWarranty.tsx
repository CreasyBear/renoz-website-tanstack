...
const submitWarrantySchema = z
	.object({
		...
		installerName: z
			.string()
			.min(2, "Installer name must be at least 2 characters"),
		...
		installPostcode: z
			.string()
			.regex(/^\d{4}$/, "Valid Australian postcode required"),
		...
		serialNumbers: z
			.array(z.string().min(1, "Serial number cannot be empty"))
		...
		installDate: z
			...
			.refine(
				...
				"Valid installation date required",
			),
		commissioningDate: z
			.string()
			.refine(
				(date) => !Number.isNaN(Date.parse(date)),
				"Valid commissioning date required",
			),
				...
				z.object({
					...
					type: z
						...
						.regex(
							...
							"Only JPEG, PNG, or PDF files allowed",
						),
				}),
			)
		...
		// Declarations
		installationDeclaration: z
			.boolean()
			.refine((val) => val === true, "Installation declaration is required"),
	...
	.refine(
		...
		{
			message:
				"Homeowner details are required when registering on their behalf",

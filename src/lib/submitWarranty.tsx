...
export const submitWarranty = createServerFn({
  ...
  if (dbError) {
    ...
    return { success: false, error: 'Failed to save warranty registration. Please try again.' }
  }
  ...
  // Email configuration
  const warrantyToEmail = process.env.WARRANTY_TO_EMAIL || import.meta.env.WARRANTY_TO_EMAIL || 'support@renoz.energy'
  const warrantyFromEmail = process.env.WARRANTY_FROM_EMAIL || import.meta.env.WARRANTY_FROM_EMAIL || 'RENOZ Energy <noreply@renoz.energy>'

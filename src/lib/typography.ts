 ...
 */
export const getTypographyClass = (level: keyof typeof typography) => typography[level];

export const combineTypography = (...classes: (keyof typeof typography | string)[]) => {
  return classes.map(cls => typeof cls === 'string' ? cls : typography[cls]).join(' ');
};
